...
import { loadBniMembers, getBniMember } from "./members.js";
...
// 若已認領 BNI 官方資料（claimedBniId），自己沒填的欄位用官方資料補齊——
// 自己填的優先，避免認領後蓋掉使用者自訂的內容。
export function userProfileToEntity(profile, user) {
  ...
  const pick = (mine, official) => mine || official || "";
  ...
  return {
    ...
    company: pick(profile.company, claimed?.company),
    categoryEn: pick(profile.category, claimed?.categoryEn),
    specialty: pick(profile.specialty, claimed?.specialty),
    region: pick(profile.region, claimed?.region),
    chapter: pick(profile.chapter, claimed?.chapter),
    business: [profile.business || claimed?.business, profile.networkCircle?.center].filter(Boolean).join("。"),
    idealReferral: pick(profile.idealReferral, claimed?.idealReferral),
    problemSolved: pick(profile.problemSolved, claimed?.problemSolved),
    idealPartner: [profile.idealPartner || claimed?.idealPartner, networkCircleText(profile.networkCircle)]
      .filter(Boolean)
      .join("、"),
    topProduct: pick(profile.topProduct, claimed?.topProduct),
    hasProfile: true,
