...
// ---- Relationships（收藏最愛 / 121 計劃：標記已認識+歸類到人脈圈分類）----
// targetId 可能含 BNI encryptedMemberId 的特殊字元(/+=)，不適合直接當 Firestore 文件 ID，
// 用子集合 + 查詢欄位比對，同認領功能的作法一致。
function relationshipsCol(userId) {
  return db.collection("profiles").doc(userId).collection("relationships");
}

export async function getRelationship(userId, targetId) {
  const snap = await relationshipsCol(userId).where("targetId", "==", targetId).limit(1).get();
  return snap.empty ? null : { id: snap.docs[0].id, ...snap.docs[0].data() };
}

export async function listRelationships(userId) {
  const snap = await relationshipsCol(userId).get();
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}

// 部分更新（收藏/已121/歸類分類），沒有就新建
export async function upsertRelationship(userId, targetId, patch) {
  const col = relationshipsCol(userId);
  const existing = await col.where("targetId", "==", targetId).limit(1).get();
  const ref = existing.empty ? col.doc() : existing.docs[0].ref;
  await ref.set({ targetId, ...patch, updatedAt: new Date().toISOString() }, { merge: true });
  const snap = await ref.get();
